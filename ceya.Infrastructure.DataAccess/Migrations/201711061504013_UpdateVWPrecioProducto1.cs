namespace ceya.Infrastructure.DataAccess.Migrations
{
    ...
    public partial class UpdateVWPrecioProducto1 : DbMigration
    {
        public override void Up()
        {
            ...
            Sql(@"CREATE VIEW [dbo].[VWPrecioProducto]
                ...
                SELECT 
                  ...
                  REPLACE(STR(Producto.Codigo, (SELECT MAX(x) FROM (VALUES (LEN(Producto.Codigo)),(4)) AS value(x))), ' ', '0')
                    + REPLACE(STR(Rubro.Codigo, (SELECT MAX(x) FROM (VALUES (LEN(Rubro.Codigo)),(2)) AS value(x))), ' ', '0')
	                + REPLACE(STR(COALESCE(Subrubro.Codigo, 0), (SELECT MAX(x) FROM (VALUES (LEN(Subrubro.Codigo)),(2)) AS value(x))), ' ', '0')
	                + REPLACE(STR(COALESCE(Clase.Codigo, 0), (SELECT MAX(x) FROM (VALUES (LEN(Clase.Codigo)),(2)) AS value(x))), ' ', '0') AS ProductoCodigoCompuesto,
                  Producto.Descripcion
                    + ' - ' + Rubro.Descripcion
	                + COALESCE(' - ' + Subrubro.Descripcion, '')
	                -- + COALESCE(' - ' + Clase.Descripcion, '')
	                + COALESCE(' - ' + Color.Descripcion, '') AS ProductoDescripcionCompuesto
                  FROM ListaPrecio
